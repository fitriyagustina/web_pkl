                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        <form action="{{ route('pkl.update', $pkl->id) }}" method="POST" enctype="multipart/form-data">
                            <a href="{{ route('pkl.create') }}" class="btn btn-md btn-success mb-3">Kembali</a>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="from-select col-md-12" name="siswa_id" id="siswa_id">
                                    ...
                                    @foreach ($data as $siswa)
                                        <option class="col-md-12" value="{{ $siswa->id }}"  {{ $pkl->siswa_id == $siswa->id ? 'selected':'' }}>{{ $siswa->nama }}
                                            </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="from-select col-md-12" name="dudi_id" id="dudi_id">
                                    ...
                                    @foreach ($item as $dudi)
                                        <option class="col-md-12" value="{{ $dudi->id }}"  {{ $pkl->dudi_id == $dudi->id ? 'selected':'' }}>{{ $dudi->nama }}
                                            </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="tgl_masuk" value="{{ $pkl->tgl_masuk }}" placeholder="">
                            </div>

                            <div class="form-group">
                                <label class="font-weight-bold">Tanggal Keluar</label>
                                <input type="date" class="form-control" name="tgl_keluar" value="{{ $pkl->tgl_keluar}}" placeholder="">
